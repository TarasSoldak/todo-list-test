...
const Geolocation: FC = () => {
  const { country, city, lat, lon, regionName, isError, isLoading } = useAppSelector(state => state.geo)
  ...
  if (isLoading) {
    ...
    <Box
      ...
      bgcolor='#1976d2'
    >
      <Typography textAlign='center' mt='23%' color='error' fontSize='1.5em'>{isError}</Typography>
    </Box>
    )
  } else {
    return (
